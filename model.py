...
class CMT(nn.Module):
    ...
    def forward(self, query, key):
        _, _, input_H, input_W = query.size()
        ...
        for query_i in range(patches_num):
            ...
            for key_j in range(patches_num):
                ...
                if query_i == key_j:
                    ...
                    i_values.append(spa_patches_value) # 
            
            i_values_tensor = torch.concat(i_values, dim=1) # [B, C * 4, H / 2, W / 2]
        
        # B, C, H, W
        B, C, H, W = i_values_tensor.size()
        spatial_reconstructed_tensor = i_values_tensor.view(B, 32, 4, H, W)
        ...
        spa_value_final = spatial_reconstructed_tensor.view(B, 32, H * 2, W * 2)

        if spa_value_final.size(2) != input_H or \
            spa_value_final.size(3) != input_W:
                upsamplied = nn.Upsample(size=(input_H, input_W))
                spa_value_final = upsamplied(spa_value_final)
...
class CMT_transformers(nn.Module):
    def __init__(self):
        ...
        self.fusionTransformer_vis = CMT(vis_flag=True, ir_flag=False)
        self.fusionTransformer_ir = CMT(vis_flag=False, ir_flag=True)
